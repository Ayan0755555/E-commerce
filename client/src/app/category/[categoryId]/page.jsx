...
const Listing = ({ params }) => {
    ...
    useEffect(() => {
        ...
        let apiEndPoint = `/api/products?category=${id}&location=${localStorage.getItem("location")}`;
    ...
    const filterByPrice = (price, catId, subCatId) => {
        ...
        if (filterId === "") {
            if (catId !== "" && catId !== null && catId !== undefined) {
                fetchDataFromApi(`/api/products?minPrice=${price[0]}&maxPrice=${price[1]}&catId=${catId}&location=${localStorage.getItem("location")}`).then((res) => {
                    setProductData(res)
                    setisLoading(false);
                    // window.scrollTo({
                    //     top: 0,
    ...
    const filterByRating = (rating, catId, subCatId) => {
        ...
        if (catId !== "" && catId !== null && catId !== undefined) {
            fetchDataFromApi(`/api/products?rating=${rating}&category=${catId}&location=${localStorage.getItem("location")}`).then((res) => {
                setProductData(res)
                setisLoading(false);
            })
        }
